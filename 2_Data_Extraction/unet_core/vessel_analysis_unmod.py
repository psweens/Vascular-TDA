...
import csv
import os
from uio import ImageReader
import json
from data_utils import im_smooth
...
class SkeletonComponent(object):
    ...
    def analyse(self):
        for n1, n2, b in self.graph.edges(data=True):
            b['attr_dict']['branch'].analyse(pix_dim=self.pix_dim)
    ...
    def remove_node_merge_branches(self, node):
        ...
        e1 = list(self.graph.get_edge_data(node, n1).items())[0][1]['attr_dict']['branch']
        e2 = list(self.graph.get_edge_data(node, n2).items())[0][1]['attr_dict']['branch']
    ...
    def collapse_branch(self, branch):
        ...
        for b in self.graph.edges(data=True):
            if b[2]['attr_dict']['branch'] == branch:
                n1_merge = b[0]
                n2_merge = b[1]
                break
        ...
        for n1, n2, data in self.graph.edges(data=True):
            ...
            if n1 in nodes:
                self.add_branch(new_node, n2, data['attr_dict']['branch'])
            elif n2 in nodes:
                self.add_branch(n1, new_node, data['attr_dict']['branch'])
    ...
    def remove_self_loops(self):
        ...
        for n1, n2, data in list(self.graph.edges(data=True)):
            if n1 is n2:
    ...
    def branch_iter(self):
        for u,v,b in list(self.graph.edges(data=True)):
            branch = b['attr_dict']['branch']
            yield u,v,branch
...
class VesselTree(object):
    ...
    def analyse_vessels(self, debug=None, verbose=True, compute_distance=True):
        ...
        self.skeleton_image = remove_small_holes(self.skeleton_image > 0, in_place=True, area_threshold=500)
    ...
    def plot_vessels(self, metric=None, metric_scaling=1, cmap='viridis', width_metric='diameter', width_scaling=0.5,
                     plot_nodes=True, write_location=None, subsample=1, threshold=None, maxval=None, minval=None):
            ...
            for c in self.skeleton.components:
                for u,v,b in c.branch_iter():
                    ...
                    if metric is None:
                        ...
                        m = int(sorted((0, m, 255.0))[1])
                        C = c_m[m]
                        C = (C[0], C[1], C[2], 1.0)
                        # C = pg.glColor('r')
                        width = b.param_dict[width_metric] * width_scaling
                        width = int(sorted((1, width, 255.0))[1])
                ...
                if valid_path:
                    ...
                    for (p1, p2) in zip(path[:-1], path[1:]):
                        ...
                        if not isinstance(b, Branch):
                            b = list(b.values())[0]
                        b['attr_dict']['branch'].used = True
                        branch.append_branch(b['attr_dict']['branch'])
...
class SkeletonTracker(object):
        ...
        for n in c.graph.nodes():
            ...
            if len(nb) == 1:
                ...
                for _e in list(e.items()):
                    min_length = max(self.min_branch_length, 2*np.max(_e[1]['attr_dict']['branch'].diameter))

                    if _e[1]['attr_dict']['branch'].length < min_length and c.num_nodes > 2:
    ...
    def find_node(self, location):
        best_dist = np.inf
        best_node = None
        for n in self.current_component.graph.nodes():
