function [handle] = plot_barcodes_javaPlex(intervals, options)
% INPUT:
...
%   it also saves it to a file. It also returns a handle to the figure drawn.
    ...
    for dimension = min_dimension:max_dimension
        ...
        if (exist('caption', 'var'))
            title([char(caption) ' (dimension ' char(num2str(dimension)) ')'],'Interpreter', 'none','Fontsize',16)
            %title(sprintf('%s (dimension %d)', caption, dimension));
        else
            ylabel(sprintf('Dim %d', dimension),'Fontsize',16);
        end
    end
